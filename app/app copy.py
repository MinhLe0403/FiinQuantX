...
# ==============================================================================
with tab_market:
    st.subheader("⚡ HỆ THỐNG ĐỊNH VỊ THỊ TRƯỜNG")
    
    # -----------------------------------------------------------
    # 1. MARKET MATRIX (DÙNG ST.CONTAINER THAY HTML)
    # -----------------------------------------------------------
    target_indices = ["VNINDEX", "VN30", "HNXINDEX", "UPCOMINDEX"]
    cols = st.columns(4)
    
    for idx, symbol in enumerate(target_indices):
        with cols[idx]:
            mk_data = market_engine.analyze_index_pro(symbol)
            
            # Khung chứa có viền (Card View chuẩn Streamlit)
            with st.container(border=True):
                if mk_data:
                    ...
                    st.metric(
                        label="Giá đóng cửa",
                        value=f"{mk_data['close']:,.2f}",
                        delta=f"{mk_data['change_pct']:+.2f}%",
                        label_visibility="collapsed" # Ẩn nhãn cho gọn
                    )
                    
                    # 2. Volume & Regime (Dùng màu Markdown :color[])
                    vol_pct = (mk_data['volume'] / mk_data['avg_volume']) * 100
                    st.caption(f"Vol: {mk_data['vol_str']} ({vol_pct:.0f}% TB)")
                    
                    st.divider() # Kẻ ngang
                    
                    # 3. Xu hướng & Điểm số
                    # Mapping màu backend sang màu Streamlit
                    trend_map = {
                        "#00CC96": "green", "#90EE90": "green", 
                        "#EF553B": "red", "#FF4444": "red", 
                        "#FFD700": "orange", "#FFA15A": "orange"
                    }
                    t_color = trend_map.get(mk_data['regime_color'], "gray")
                    
                    st.markdown(f"Xu hướng: :{t_color}[**{mk_data['regime']}**]")
                    
                    # 4. Progress Bar cho Health Score
                    score = mk_data['health_score']
                    st.markdown(f"Sức mạnh: **{score}/100**")
                    st.progress(score / 100, text=None)
                    
                    # 5. Phân phối
                    dist_d = mk_data['dist_days']
                    dist_txt = f"{dist_d} ngày phân phối"
                    if dist_d >= 4:
                        st.markdown(f":red[⚠️ {dist_txt}]")
                    else:
                        st.caption(f"✅ {dist_txt}")
                        
                else:
                    st.warning(f"{symbol}: No Data")
                    st.caption("⚠️ Chưa tải dữ liệu")

    # === [NEW] MARKET FLOW ENGINE (MFE) DASHBOARD ===
    # Phân tích sâu dòng tiền thông minh của VNINDEX
    mfe_data = market_engine.analyze_market_flow_pro("VNINDEX")
    
    if mfe_data:
        st.markdown("### 🌊 ĐỘNG CƠ DÒNG TIỀN (MARKET FLOW ENGINE)")
        
        c_main, c_comps = st.columns([1, 2])
        
        with c_main:
            # Hiển thị điểm số tổng
            score = mfe_data['mfe_score']
            color = "#00CC96" if score >= 70 else "#EF553B" if score <= 40 else "#FFD700"
            
            st.markdown(f"""
            <div style="text-align: center; border: 4px solid {color}; border-radius: 50%; width: 180px; height: 180px; margin: 0 auto; display: flex; flex-direction: column; justify-content: center; align-items: center; background-color: #1E1E1E;">
                <span style="color: #aaa; font-size: 14px;">MFE SCORE</span>
                <strong style="font-size: 48px; color: {color}; line-height: 1;">{score}</strong>
                <span style="color: {color}; font-size: 12px; font-weight: bold;">
                    {"TIỀN VÀO MẠNH" if score>=70 else "TIỀN RÚT/YẾU" if score<=40 else "TRUNG TÍNH"}
                </span>
            </div>
            """, unsafe_allow_html=True)
            
            # FSI Status
            fsi_raw = mfe_data['raw']['fsi_val']
            fsi_lbl = mfe_data['raw']['fsi_label']
            if abs(fsi_raw) > 2.0:
                st.info(f"💡 FSI Signal: **{fsi_lbl}** (Z={fsi_raw:.1f})")

        with c_comps:
            # Hiển thị các thành phần (Factor Analysis)
            comps = mfe_data['components']
            
            # 1. Smart Money Factor
            st.write("**1. Dòng tiền Thông minh (Smart Money):**")
            st.progress(comps['smf']/100)
            st.caption(f"Đánh giá gom ròng của Khối ngoại & Tự doanh. Điểm: {comps['smf']}/100")
            
            # 2. Đồng thuận (Divergence)
            st.write("**2. Sự đồng thuận (Price-Flow Correlation):**")
            st.progress(comps['div']/100)
            st.caption(f"Mức độ đồng pha giữa giá và dòng tiền. Điểm: {comps['div']}/100")
            
            # 3. Độ an toàn (Anti-Fragility)
            st.write("**3. Độ Vững chãi (Market Structure):**")
            st.progress(comps['fragility']/100)
            st.caption(f"Khả năng chống chịu cú sốc thanh khoản. Điểm: {comps['fragility']}/100")

    st.divider()
    st.markdown("---")
    st.subheader("🧪 PHÒNG LAB ĐỊNH LƯỢNG (QUANT LAB)")
    
    # Chỉ số phân tích sâu cho VNINDEX
    q_data = market_engine.get_quant_metrics("VNINDEX")
    
    if q_data:
        q1, q2, q3, q4 = st.columns(4)
        
        # 1. LIQUIDITY (Thanh khoản động)
        with q1:
            st.markdown("**1. Động lực Thanh khoản**")
            z = q_data['liq_z_score']
            z_col = "red" if z < -1.5 else "green" if z > 1.5 else "gray"
            st.metric("Liquidity Z-Score", f"{z:+.2f} σ", delta=q_data['liq_status'], delta_color="off")
            st.caption(f"Trạng thái dòng tiền so với TB 20 phiên. > 2.0 là Tiền vào sốc.")

        # 2. MICROSTRUCTURE (Cung Cầu Chủ động)
        with q2:
            st.markdown("**2. Cấu trúc Cung/Cầu**")
            imb = q_data['imbalance']
            imb_txt = "Phe Mua Thắng" if imb > 0 else "Phe Bán Thắng"
            st.metric("Order Imbalance", f"{imb:+.1f}%", delta=imb_txt)
            
            # Progress bar visual
            val = (imb + 100)/200 # Map -100..100 to 0..1
            st.progress(val, text=None)
            st.caption("Lực Mua/Bán chủ động khớp lệnh.")

        # 3. POSITIONING (Vị thế giá)
        with q3:
            st.markdown("**3. Áp lực Giá vốn (VWAP)**")
            dev = q_data['vwap_dev']
            lbl = "Quá Mua (Căng)" if dev > 2 else "Quá Bán (Hấp dẫn)" if dev < -2 else "Cân bằng"
            color = "inverse" if abs(dev) > 2 else "off"
            st.metric("VWAP Deviation", f"{dev:+.2f} ATR", delta=lbl, delta_color=color)
            st.caption("Giá cách xa giá vốn trung bình bao nhiêu lần biến động.")

        # 4. VOLATILITY REGIME (Chế độ thị trường)
        with q4:
            st.markdown("**4. Chế độ Biến động**")
            vr = q_data['vol_regime']
            v_color = "#EF553B" if "HIGH" in vr else "#00CC96" if "LOW" in vr else "#FFA15A"
            
            st.markdown(f"""
            <div style="text-align: center; border: 2px solid {v_color}; border-radius: 8px; padding: 10px;">
                <strong style="color: {v_color};">{vr}</strong>
                <br>
                <span style="font-size: 12px; color: #888;">Rank: {q_data['vol_rank']}/100</span>
            </div>
            """, unsafe_allow_html=True)
            st.caption("Rank cao = Biến động rủi ro lớn. Rank thấp = Tích lũy nén.")
            
    else:
        st.info("Chưa đủ dữ liệu để chạy mô hình định lượng (Cần tối thiểu 50 phiên có data chi tiết).")

    # -----------------------------------------------------------
    # 2. MARKET DEEP DIVE (Lấy VNINDEX làm chuẩn)
    # -----------------------------------------------------------
    st.markdown("---")
    mk_main = market_engine.analyze_index_pro("VNINDEX")
    
    if mk_main:
        c_breadth, c_flow, c_sec = st.columns([1.5, 1.5, 2.5])
        
        # --- Cột 1: Độ rộng thị trường (Breadth) ---
        with c_breadth:
            with st.container(border=True):
                st.markdown("##### 🩺 Độ rộng")
                br = mk_main.get('breadth', {})
                breadth_real = market_engine.get_market_breadth()
                
                # Biểu đồ Pie Chart
                if breadth_real:
                    green, red, yellow = breadth_real.get('green',0), breadth_real.get('red',0), breadth_real.get('yellow',0)
                    ...
                    if total > 0:
                        fig_pie = go.Figure(data=[go.Pie(
                            ...
                            hole=.6, 
                            marker_colors=['#00CC96', '#EF553B', '#FFD700'],
                            textinfo='none'
                        )])
                        # Số liệu tổng hợp ở giữa
                        green_pct = int(green/total*100)
                        fig_pie.update_layout(
                            annotations=[dict(text=f"{green_pct}%", x=0.5, y=0.5, font_size=24, showarrow=False, font_color="#00CC96")],
                        ...
                        # Text Stats bên dưới
                        c1, c2 = st.columns(2)
                        c1.metric("Tăng", f"{green}")
                        c2.metric("Giảm", f"{red}")

                st.divider()
                st.caption(f"• > SMA200: **{br.get('pct_above_sma200',0)}%**")
                st.caption(f"• > SMA50: **{br.get('pct_above_sma50',0)}%**")
                st.caption(f"• Vượt đỉnh 52W: **{br.get('near_high_52w',0)} mã**")

        # --- Cột 2: Dòng tiền Vốn hóa (Rotation) ---
        with c_flow:
            with st.container(border=True):
                st.markdown("##### 💰 Dòng tiền")
                rot = mk_main.get('rotation_data')
                
                if rot is not None and not rot.empty:
                    # Bar chart ngang đơn giản
                    fig_bar = go.Figure(go.Bar(
                        x=rot['total_val'], 
                        y=rot['cap_group'], 
                        orientation='h',
                        marker_color=['#AB63FA', '#FFA15A', '#19D3F3'],
                        text=rot['total_val'].apply(lambda x: f"{x/1e9:.0f}"),
                        textposition='auto'
                    ))
                    fig_bar.update_layout(
                        height=200, 
                        margin=dict(t=0,b=0,l=0,r=0), 
                        yaxis=dict(showticklabels=False), # Ẩn nhãn trục Y
                    ...
                    <div style="font-size: 11px; display: flex; justify-content: space-around;">
                        <span style="color:#AB63FA">■ Large</span>
                        <span style="color:#FFA15A">■ Mid</span>
                        <span style="color:#19D3F3">■ Small</span>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Insight
                    top = rot.iloc[0]['cap_group'].split('(')[0]
                    st.info(f"Tập trung: **{top}**")
                else:
                    st.warning("Thiếu data.")
                    st.caption("Hãy chạy 'Tải VNINDEX'.")

        # --- Cột 3: Top Sector ---
        with c_sec:
            with st.container(border=True):
                st.markdown("##### 🔥 Top Ngành")
                df_sec = sector_engine.get_sector_ranking(limit_days=5)
                
                if not df_sec.empty:
                    # Chuẩn hóa tên cột
                    pct_col = 'weighted_change_pct' if 'weighted_change_pct' in df_sec.columns else 'avg_change_pct'
                    score_col = 'composite_score' if 'composite_score' in df_sec.columns else None
                    
                    # Config cột
                    cols_cfg = {
                        "sector": st.column_config.TextColumn("Nhóm Ngành"),
                        pct_col: st.column_config.NumberColumn("% Tăng", format="%.2f%%"),
                        "total_flow": st.column_config.NumberColumn("GTGD (Tỷ)", format="%.0f"),
                    }
                    if score_col:
                        cols_cfg[score_col] = st.column_config.ProgressColumn("Sức mạnh", min_value=0, max_value=10, format="%.1f")

                    st.dataframe(
                        df_sec.head(6),
                        use_container_width=True,
                        column_order=["sector", pct_col, "total_flow", score_col] if score_col else ["sector", pct_col, "total_flow"],
        ...
        st.warning("Vui lòng tải dữ liệu VNINDEX ở thanh bên trái (Tab Theo Lô -> Tải VNINDEX) để kích hoạt Dashboard.")
# ==============================================================================
# TAB 2: PHÂN TÍCH CỔ PHIẾU (STOCK ANALYSIS)
# ==============================================================================
with tab_stock:
    # A. Validate Input
    if not input_symbol:
